 ...
 * You can navigate around the filesystem, moving into or out of directories and listing the contents of the directory
 * you're currently in.
 *
 ...
 * which isn't quite the 30000000 required by the update. Therefore, the update still requires a directory with total size of at least 8381165 to be deleted before it can run.
 *
...
fun main() {
    ...
    fun part1(input: List<String>): Long {
        ...
        val dirs = traverser.extractDirs()

        return dirs.map { it.getFullSize(dirs) }.filterNot { it > MAX }.sum()
    }
    ...
    fun part2(input: List<String>): Long {
        ...
        val availableSpace = DISK_SIZE - dirs.first().getFullSize(dirs)
        val missingSpace = REQUIRED_SPACE - availableSpace

        return dirs.map { it.getFullSize(dirs) }.sorted().first { it > missingSpace }
    }
